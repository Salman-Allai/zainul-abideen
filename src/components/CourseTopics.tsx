  ...
  return (
    <section id="course-topics" className="py-10 px-2 sm:px-4 md:px-6">
      <div className="container mx-auto">
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-purple-800 text-center mb-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {courseCategories.map((category) => (
            <div
              key={category.id}
              className="bg-white rounded-xl shadow p-4 sm:p-6 flex flex-col items-center text-center hover:shadow-lg transition-shadow"
            >
              <div className="text-3xl sm:text-4xl mb-2">{category.icon}</div>
              <h3 className="text-lg sm:text-xl md:text-2xl font-semibold text-purple-700 mb-1">{category.title}</h3>
              <p className="text-base sm:text-lg text-black-700">{category.description}</p>
            </div>
          ))}
        </div>
